...
10. [NEXT] Validate `usb_xinput` end to end from the Raspberry Pi host path.
   ...
   - [BLOCKED] Flash and exercise the new runtime instrumentation on hardware. Current blockers: `/dev/ttyACM0` still disappears before the second-stage firmware upload on the old flow, and the fallback GPIO-JTAG attach most recently failed with OpenOCD scan-chain examination errors (`all ones`, `Unexpected OCD_ID = ffffffff`).
